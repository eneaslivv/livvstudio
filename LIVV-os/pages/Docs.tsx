...
interface DocFile {
  ...
  folderId: string;
  name: string;
  type: 'doc' | 'code' | 'sheet' | 'image' | 'pdf' | 'zip';
  size: string;
  author: string;
  avatar: string;
  date: string;
...
const FolderGraphic = ({ icons, count }: { icons?: string[], count: number }) => (
    <div className="relative w-14 h-11 group-hover:scale-105 transition-transform duration-300">
      <div className="absolute top-0 right-0 w-[56px] h-[44px] bg-zinc-200 dark:bg-zinc-700 rounded-md"></div>
      <div className="absolute top-[-4px] left-[6px] w-[44px] h-[36px] bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 shadow-sm rounded-[2px] transform -rotate-3"></div>
      <div className="absolute bottom-0 left-0 w-[56px] h-[36px] bg-zinc-300 dark:bg-zinc-600 rounded-b-md rounded-tr-md shadow-sm z-10 flex items-end justify-end p-1 gap-0.5">
...
export const Docs: React.FC<DocsProps> = ({ initialFolderId = 'root' }) => {
  ...
  useEffect(() => { setCurrentFolderId(initialFolderId); }, [initialFolderId]);
  ...
  const handleSaveFile = (title: string, content: string, existingId?: string) => {
    if (existingId) {
      setFiles(prev => prev.map(f => f.id === existingId ? { ...f, name: title + (f.type === 'doc' ? '.docx' : '.tsx'), content, date: 'Just now' } : f));
    } else {
      const newFile: DocFile = {
        id: Date.now().toString(), folderId: currentFolderId, name: title + (editorType === 'doc' ? '.docx' : '.tsx'),
        type: editorType, size: '1 KB', author: 'Eneas', avatar: 'E', date: 'Just now', content
      };
  ...
  const startEdit = (file: DocFile) => {
    setEditorType(file.type as 'doc' | 'code');
    setEditingFile(file);
