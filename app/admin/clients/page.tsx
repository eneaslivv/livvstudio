...
import { adaptClient } from '@/lib/admin-adapters';
...
const AddClientModal = ({ isOpen, onClose, onAdd }: { isOpen: boolean, onClose: () => void, onAdd: (c: Client, invite: boolean) => void }) => {
    ...
    const handleSubmit = async () => {
        ...
        await onAdd(newClient, sendInvite);
        setIsInviting(false);
            ...
            <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-800 p-6">
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2 pt-2">
                        <input type="checkbox" id="invite" checked={sendInvite} onChange={e => setSendInvite(e.target.checked)} className="w-4 h-4 rounded border-zinc-300 text-indigo-600 focus:ring-indigo-500" />
                        <label htmlFor="invite" className="text-sm text-zinc-600 dark:text-zinc-400 select-none">Send invitation email with access key</label>
                    </div>
                </div>
                <div className="flex justify-end gap-2 mt-6">
                    <button onClick={onClose} disabled={isInviting} className="px-4 py-2 text-sm text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-300 disabled:opacity-50">Cancel</button>
                    <button onClick={handleSubmit} disabled={isInviting} className="px-4 py-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-md text-sm font-medium flex items-center gap-2 disabled:opacity-70">
                        {isInviting && <Icons.Loader className="animate-spin" size={14} />}
                        {isInviting ? 'Processing...' : 'Add Contact'}
                    </button>
                </div>
            </div>
        </div>
                ...
                {client.messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[70%] flex flex-col ${msg.sender === 'me' ? 'items-end' : 'items-start'}`}>
                            <div
                                className={`px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.sender === 'me'
...
export default function ClientsPage() {
    ...
    // Integration: Fetch Leads as Clients
    const { data: dbLeads, loading, add: addLead } = useSupabase<any>('leads');

    const realClients = useMemo(() => {
        if (dbLeads.length > 0) {
            return dbLeads.map(adaptClient);
        }
        return [];
    }, [dbLeads]);
    ...
    useEffect(() => {
        if (realClients.length > 0) {
            setClients([...realClients, ...MOCK_CLIENTS.filter(mc => !realClients.find((rc: Client) => rc.email === mc.email))]);
            if (!selectedClientId || selectedClientId === '1') {
                // Keep selection logic simple
            }
        }
    }, [realClients, selectedClientId]);


    const selectedClient = clients.find(c => c.id === selectedClientId) || clients[0] || MOCK_CLIENTS[0];

    // --- ACTIONS ---

    const handleAddClient = async (newClient: Client, sendInvite: boolean) => {
        const payload = {
            name: newClient.name,
            email: newClient.email,
            company: newClient.company,
            status: 'new',
            // Storing role in message temporarily as there is no column
            message: `Role: ${newClient.role}`
        };

        await addLead(payload);

        if (sendInvite && newClient.email) {
            try {
                const { error } = await supabase.functions.invoke('invite-client', {
                    body: { email: newClient.email, name: newClient.name, role: newClient.role }
                });
                if (error) {
                    console.error("Invite invocation error:", error);
                    // Don't block UI
                }
            } catch (err) {
    ...
    if (!selectedClient) return <div className="p-8 text-center text-zinc-400">Loading Clients...</div>;
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                    {filteredClients.map(client => (
                        <div
                            ...
                            className={`group flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all ${selectedClientId === client.id
                        ...
                        <div className="h-full overflow-y-auto p-8 grid grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                    ...
                                    <div className="space-y-6 relative pl-2 border-l border-zinc-200 dark:border-zinc-800 ml-2">
                                        {selectedClient.history.length > 0 ? selectedClient.history.map(item => (
                                            <div key={item.id} className="relative pl-6">
                                                <div className={`absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full ring-4 ring-white dark:ring-zinc-900 ${item.type === 'meeting' ? 'bg-orange-400' :
                            ...
                            <div className="col-span-1 border-l border-zinc-100 dark:border-zinc-800 pl-8 space-y-6">
                                ...
                                <div className="pt-6 border-t border-zinc-100 dark:border-zinc-800">
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center gap-3 p-2 rounded hover:bg-zinc-50 dark:hover:bg-zinc-800 cursor-pointer transition-colors border border-transparent hover:border-zinc-200 dark:border-zinc-700">
