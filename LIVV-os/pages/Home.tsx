...
import { Project, Status, Collaborator, PageView, Priority } from '../types';
import { AIPlanner } from '../components/AIPlanner';

// --- MOCK DATA ---
const INITIAL_PRIORITIES = [
  { id: '1', title: 'Review Q3 Strategy for Client Alpha', completed: false, tag: 'Strategy', priority: Priority.High, estimatedHours: 2 },
  { id: '2', title: 'Finalize Architecture for SaaS Project', completed: false, tag: 'Dev', priority: Priority.Medium, estimatedHours: 4 },
  { id: '3', title: 'Call with Design Team (Lucas)', completed: true, tag: 'Meeting', priority: Priority.Low, estimatedHours: 1 },
];

const INITIAL_MICROTASKS = [
  'Invoice #402',
  'Reply Slack',
  'Update Linear'
];
...
const ACTIVE_PROJECTS: Project[] = [
  { 
    id: '1', title: 'Fintech Dashboard', description: 'UX Overhaul', progress: 78, status: Status.Active, nextSteps: 'User testing phase', tasks: [], updatedAt: '2h ago', client: 'Bank Corp'
  },
  { 
    id: '2', title: 'E-commerce API', description: 'Backend integration', progress: 32, status: Status.Pending, nextSteps: 'Database schema review', tasks: [], updatedAt: '1d ago', client: 'Shopifyy'
  },
  { 
    id: '3', title: 'Personal Brand', description: 'Website refresh', progress: 92, status: Status.Review, nextSteps: 'Final content polish', tasks: [], updatedAt: '4h ago', client: 'Me'
  },
  { 
    id: '4', title: 'Internal Wiki', description: 'Documentation', progress: 15, status: Status.Active, nextSteps: 'Structure', tasks: [], updatedAt: '5d ago', client: 'Internal'
  }
];

const COLLABORATORS: Collaborator[] = [
  { id: '1', name: 'Sofia R.', role: 'Designer', avatarInitials: 'SR', status: 'online', pendingTasks: 2 },
  { id: '2', name: 'Lucas M.', role: 'Product Manager', avatarInitials: 'LM', status: 'busy', pendingTasks: 4 },
  { id: '3', name: 'Miguel T.', role: 'Frontend', avatarInitials: 'MT', status: 'offline', pendingTasks: 0 },
];
...
export const Home: React.FC<HomeProps> = ({ onNavigate, onStartTimer, activeTimerId }) => {
  // State
  const [tasks, setTasks] = useState(INITIAL_PRIORITIES);
  const [microTasks, setMicroTasks] = useState(INITIAL_MICROTASKS);
  ...
  const addMicroTask = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && newMicroTask.trim()) {
        setMicroTasks([...microTasks, newMicroTask.trim()]);
        setNewMicroTask('');
    }
  };
  ...
  const handleApplyPlan = (newTasks: any[]) => {
    ...
    alert(`${newTasks.length} tasks synchronized with your schedule.`);
  };
                       ...
                       >
                           <div className="flex items-center gap-4 flex-1" onClick={() => toggleTask(task.id)}>
                               <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
                                   task.completed ? 'bg-zinc-500 border-zinc-500 text-white' : 'border-zinc-300 dark:border-zinc-600 text-transparent'
                               }`}>
                           ...
                           <div className="flex items-center gap-4">
                               {!task.completed && (
                                   <button 
                                      ...
                                      className={`p-2 rounded-full transition-all ${activeTimerId === task.id ? 'bg-emerald-500 text-white' : 'text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100'}`}
                                   >
                    ...
                    <div className="flex flex-wrap gap-2">
                        {microTasks.map((mt, i) => (
                            <button 
                                ...
                                className="group flex items-center gap-1.5 px-3 py-1.5 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-xs font-medium text-zinc-600 dark:text-zinc-300 hover:bg-rose-50 hover:text-rose-700 hover:border-rose-200 dark:hover:bg-rose-900/20 dark:hover:text-rose-300 transition-all"
                            >
                                {mt}
